...
from strokeFormer.utils.util import get_scheduler,mkdir,create_masks,create_padding_mask
from strokeFormer.utils.strokeTool import to_normal_strokes
from strokeFormer.utils.schedulers import Scheduler

from strokeFormer.models.model.transformer import Transformer
...
class SketchModel:
    ...
    def build_model(self,opt):
        self.model = Transformer(enc_voc_size=opt.src_vocab,dec_voc_size=opt.tgt_vocab,d_model=opt.d_model,
                                max_len=opt.stroke_len,ffn_hidden=opt.dim_feedforward,n_head=opt.nhead,n_layers=opt.num_layers,
    ...
    def make_dummy_input(self, nattn, batch_size):
        ...
        nignore = self.stroke_len - nattn

        l = torch.zeros((1,self.opt.tgt_vocab)).squeeze()
        r = torch.zeros((1,self.opt.tgt_vocab)).squeeze()
        r[-1] = 1

        dummy = torch.cat((torch.ones((batch_size, nattn, self.opt.tgt_vocab)) * torch.tensor(l,dtype=torch.float32),
            torch.ones((batch_size, nignore, self.opt.tgt_vocab)) * torch.tensor(r,dtype=torch.float32)
        ), axis=1)
    ...
    def predict_from_embedding(self,emb):
        ...
        with torch.no_grad():
            ...
            for i in range(self.stroke_len):
                nattn =  i + 1
    ...
    def load_network(self, epoch, mode='test'):
        ...
        """
        model_path = os.path.join('strokeFormer/',self.save_dir, '{}_transformer_{}.pkl'.format(self.net_name, epoch))
        #model_path = os.path.join(self.save_dir, '{}_transformer_{}.pkl'.format(self.net_name, epoch))
