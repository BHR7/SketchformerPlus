...
def to_normal_strokes(big_stroke):  #batch_size x seq_len x 4
    ...
    for stroke in big_stroke:  #to every segment in big_stroke
        r = 0
        for i in range(1, len(stroke)):
            if np.argmax(stroke[i, :]) == 3:
                r = i
                break
        if r == 0:
            r = len(stroke)

        result[l:l + r - 1,0:2] = stroke[1:r, 0:2]
        #result[l+r-1,2] = 1
...
def to_normal_sketch(big_sketch):
    return
